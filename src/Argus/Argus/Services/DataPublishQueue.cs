...
namespace Argus
{
    public class DataPublishQueue
    {
        ...
        private void PublishData(DataToPublish dataToPublish)
        {
            try
            {
                var sequenceNumber = _messageQueueChannel.NextPublishSeqNo;
                _logger.LogInformation(
                    $"Service '{_serviceName}' starts to publish data generated by service '{dataToPublish.ServiceProcedure.Service}' procedure '{dataToPublish.ServiceProcedure.Procedure}' with sequence number '{sequenceNumber}'");

                var data = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(dataToPublish));
        ...
        private void ProcessBasicNack(ulong sequenceNumber)
        {
            ...
            var errorMessage = 
                $"failed to publish data generated by service '{dataToPublish.ServiceProcedure.Service}' procedure '{dataToPublish.ServiceProcedure.Procedure}' with sequence number '{sequenceNumber}' as it is nack-ed by the message queue.";
